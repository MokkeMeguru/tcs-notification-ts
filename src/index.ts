...
const cronSendNotifications: CronSendNotifications = async psqlclient => {
  ...
  res.forEach(r => {
    ...
    let message = {
      notification: {
        title: `Push Notification from ${appname}`, // TODO: write
        body: `Are you ${r.name} ?`, // TODO: fill task name
        icon: "assets/main-page-logo-small-hat.png",
        vibrate: [100, 50, 100],
        data: { message: `Hello! You have a task.` },
    ...
    webpush
      ...
      .then(_ =>
        console.log(`correct push for ${r.name}-${r.endpoint}-${message}`)
      )
